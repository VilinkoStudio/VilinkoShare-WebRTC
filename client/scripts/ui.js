...
Events.on('display-name', e => {
    ...
    if ($displayName) {
        $displayName.textContent = '您的显示名称：' + me.displayName;
...
class PeerUI {
    ...
    html() {
        return `
            <label class="column center" title="点击发送文件或右键发送消息">
    ...
    _onFilesSelected(e) {
        ...
        console.log('PeerUI: files selected (文件已选择):', files.length, 'to (发送到):', this._peer.id);
        Events.fire('files-selected', {
            files: files,
            to: this._peer.id
        });
...
class ReceiveDialog extends Dialog {
    ...
    _displayFile(file) {
        ...
        if(file.mime.split('/')[0] === 'image'){
            console.log('the file is image (文件是图片)');
...
class NetworkStatusUI {
    ...
    _showOnlineMessage() {
        Events.fire('notify-user', '您已重新上线');
    }
}
...
class WebShareTargetUI {
    constructor() {
        ...
        console.log('Shared Target Text (共享目标文本):', '"' + shareTargetText + '"');
    }
}
...
const vilinkoStudio = new VilinkoStudio();
...
if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('service-worker.js')
        .then(serviceWorker => {
            console.log('Service Worker registered (Service Worker已注册)');
